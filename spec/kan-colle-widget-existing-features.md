...
利用者はオプションページからフレーム設定の追加・削除・初期化を行い、自分のディスプレイ環境に合わせた別窓を登録できる。所属サーバに応じて必要な `http://xxx/*` 権限と `<all_urls>` 権限を付与・解除でき、全サーバ一括操作も可能。最新リリース情報や告知は JSON から読み込み一覧表示される。初回インストール時に権限が未付与なら自動的に設定ページが開かれる。

**Why this priority**: 安全な権限管理とユーザー自身によるカスタマイズが利用継続の鍵であり、リリース情報の透明性も信頼性向上に直結する。

**Independent Test**: オプションページを開き、(1) 別窓を新規登録して再表示されること、(2) サーバ権限を切り替えた後に `chrome.permissions` が更新されること、(3) リリースノートのリンクが外部リポジトリへ遷移すること、(4) 初期状態で権限を全て外したのち拡張を再読み込みすると設定ページが自動表示されること、を確認する。

**Acceptance Scenarios**:

1. **Given** 別窓を起動している, **When** 「今開いている別窓を新規設定として登録」を選択する, **Then** 現在のサイズとズームが新規フレームとして保存され一覧へ即時追加される。
2. **Given** フレーム設定一覧が表示されている, **When** 保護されていない設定を削除する, **Then** リストから消え再読み込み後も復活しない。
3. **Given** サーバ権限カードをクリックした, **When** 権限を付与または取り消す, **Then** 対応する `chrome.permissions` 状態が変更され UI も即時更新される。
4. **Given** すべてのサーバ権限を外した状態で拡張を再インストールした, **When** `onInstalled` が発火する, **Then** サーバ権限セクションを開いたオプションタブが自動で表示される。
...
- `<all_urls>` もしくは対象サーバの権限が拒否された場合、スクリーンショット取得や OCR が失敗しタイマー生成が止まるため、権限セクションで再許可を促すメッセージが求められる。
...
- **FR-002**: システムは専用ウィンドウの位置・サイズ・ズームを 10 秒間隔で記録し、次回起動時に復元しなければならない。
...
- **FR-010**: システムはサーバ権限カード操作に応じて該当 `http://<server>/*` と `<all_urls>` の許可・取消しを実行し、その結果を画面へ反映しなければならない。
- **FR-011**: システムは権限が未付与の状態でインストールされた場合、サーバ権限セクションを開いたオプションページを自動表示しなければならない。
- **FR-012**: システムは通知クリック時にゲーム別窓を起動またはフォーカスし、通知をクリアしなければならない。
- **FR-013**: システムはリリースノート JSON を読み込み、バージョン・告知・コミットリンクをオプションページへ表示しなければならない。

### Key Entities *(include if feature involves data)*

- **Frame**: 別窓起動に必要な位置・サイズ・ズーム・ URL・保護フラグを保持する永続データ。プリセットとユーザー定義の両方を扱い、記憶用の特別エントリ（`__memory__`）を持つ。
...
- **ServerPermission**: 艦これサーバごとの IP アドレスと権限付与状態を保持し、画面表示や一括操作の基礎となるデータ。
...
- **SC-005**: サーバ権限の付与・取消し操作が 1 秒以内に反映され、操作後の画面と `chrome.permissions` の状態が一致すること。
- **SC-006**: インストール直後にサーバ権限が未付与だったユーザーの 100% が自動的にオプションページへ誘導され、設定セクションを開いた状態で操作できること。
