...
利用者はオプションページからフレーム設定の追加・削除・初期化を行い、自分のディスプレイ環境に合わせた別窓を登録できる。ゲームウィンドウ内表示設定では、ミュートボタンとスクショボタンの表示/非表示、およびボタンサイズ（小・大）を個別に制御可能。最新リリース情報や告知は JSON から読み込み一覧表示される。

**Why this priority**: 安全な権限管理とユーザー自身によるカスタマイズが利用継続の鍵であり、リリース情報の透明性も信頼性向上に直結する。

**Independent Test**: オプションページを開き、(1) 別窓を新規登録して再表示されること、(2) ウィンドウ内表示設定で各ボタンの表示/非表示とサイズを変更し次回起動時に反映されること、(3) リリースノートのリンクが外部リポジトリへ遷移すること、を確認する。
...
3. **Given** ウィンドウ内表示設定で任意のボタンを非表示にした, **When** 次回ゲーム窓を起動する, **Then** 該当ボタンがゲーム画面に表示されない。
4. **Given** ウィンドウ内表示設定でボタンサイズを「小」に変更した, **When** 次回ゲーム窓を起動する, **Then** ボタンが 24px × 24px で表示される。
5. **Given** リリースノート一覧が表示されている, **When** 任意のコミットリンクをクリックする, **Then** 対応する GitHub コミットページが新規タブで開く。
...
- **FR-010**: システムはオプションページでウィンドウ内表示設定（ミュート・スクショボタンの表示/非表示、ボタンサイズ）を提供し、次回ゲーム窓起動時に反映しなければならない。
- **FR-011**: システムは通知クリック時にゲーム別窓を起動またはフォーカスし、通知をクリアしなければならない。
- **FR-012**: システムはリリースノート JSON を読み込み、バージョン・告知・コミットリンクをオプションページへ表示しなければならない。
...
- **GameWindowConfig**: ゲーム窓の動作設定を保持する永続データ。閉じる前の確認ダイアログ有無 (`alertBeforeClose`)、ウィンドウ内ボタンの表示制御 (`showMuteButton`, `showScreenshotButton`)、ボタンサイズ (`buttonSize`: 50=小, 100=大) を管理する。
...
- `src/injection/dmm.ts` が `play.games.dmm.com` 側へ注入された直後に
  `InAppActionButtons.create(config)` を呼び出し、1 ウィンドウにつき 1 インスタンスだけ
  を生成する。
...
- コンテナ内部には最大 2 つのボタンが横並びで生成され、右から順にミュート、左が
  スクリーンショット。各ボタンの表示/非表示は `GameWindowConfig` で制御可能。
- ボタンは背景色白、ボーダー無し。デフォルトサイズは 48px × 48px (padding: 8px) だが、
  `GameWindowConfig.buttonSize` により小サイズ (50%: 24px × 24px) または大サイズ (100%: 48px × 48px)
  を選択可能。マウスオーバーでカーソルがポインタになる。
- ミュートボタンはタブの音声状態に応じて Heroicons のスピーカーアイコン
  (ICON_SPEAKER_WAVE/ミュート解除) とスピーカー消音アイコン (ICON_SPEAKER_X_MARK/ミュート) を切り替える。
- スクリーンショットボタンは Heroicons のカメラアイコン (ICON_CAMERA) を表示する。

### 操作シーケンス

- スクリーンショット
  - クリック即時にコンテナを DOM から一時的に削除し、
    Chrome の描画タイミングを考慮して 2 フレーム待機した後に `/screenshot` メッセージをバックグラウンドへ送信する。
  - 撮影完了後にコンテナを DOM に復元し、撮影画像への写り込みを抑制する。
- ミュート切替
...
- InAppActionButtons は Chrome 拡張 API を前提とし、`sender.tab` が取得できない
  メッセージ経路では動作しない。
- トップレベルの `dmm.ts` が再注入されても `create(config)` が既存インスタンスを
  再利用し、重複生成を防ぐ。
- 画面内の他要素を阻害しないよう、背景は半透明白でクリック透過はしないが、
  ミュートや撮影以外の機能を提供しない。
- ボタンの表示/非表示およびサイズは `GameWindowConfig` (`showMuteButton`, `showScreenshotButton`, `buttonSize`)
  に依存し、設定変更は次回ゲームウィンドウ起動時に反映される。
...
- ミュート切替後に Chrome のタブ音量状態が即時反映され、アイコン表示が状態と一致
  すること。
- 連続クリック時にもエラーが発生せず、メッセージ送信が正しいタブに紐付くこと。
- オプションページで各ボタンの表示/非表示を切り替え、次回起動時に設定が反映されること。
- オプションページでボタンサイズを「小」「大」で切り替え、次回起動時に正しいサイズで表示されること。
