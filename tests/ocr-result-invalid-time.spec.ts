...
// OCRが時刻を読み損なった場合、誤った scheduled の Queue を登録せず、通知も出さないことを
// 検証する。同じドックの既存Queueの掃除（deleteSlot）は修復・建造が始まった事実に基づくので、
// OCR結果の成否に依らず行われること。
describe("OCR結果が時刻として解釈できない場合はタイマーを登録しない", () => {
  ...
  it.each([
    ...
    ["余剰セグメント", "1:2:3:4"],
    ["分秒の範囲外", "0:99:99"],
  ])("修復OCR結果が不正(%s: %j): 既存Queueの掃除だけ行い、Queueを作らず通知もしない", async (_label, text) => {
    await dispatch({
    ...
    expect(deleteSlot).toHaveBeenCalledWith(EntryType.RECOVERY, "1");
    expect(create).not.toHaveBeenCalled();
    expect(notify).not.toHaveBeenCalled();
  });

  it("建造OCR結果が不正: 既存Queueの掃除だけ行い、Queueを作らず通知もしない", async () => {
    await dispatch({
  ...
  it("末尾に改行が付いたOCR結果は正常として扱う（Tesseractの実出力形式）", async () => {
    await dispatch({
      __action__: `/injected/dmm/ocr/${EntryType.RECOVERY}:result`,
      data: { text: "00:00:30\n" },
      [EntryType.RECOVERY]: { dock: "1" },
