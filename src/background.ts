...
// kcsapi リクエスト Recorder（ローカル開発専用の可観測性機能 / #1790, #1793）。
// dev ビルドで localhost host_permission が注入されているときだけ有効。既存ルーターとは独立に
// 登録し、ハンドルされない kcsapi も含めて全件を localhost dev サーバへ送って記録する。
